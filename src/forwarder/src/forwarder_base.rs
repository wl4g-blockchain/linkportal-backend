...
use linkportalbackend_server::{config::config, context::state::LinkPortalBackendState, util::auths};
use linkportalbackend_types::forwarder::HttpIncomingRequest;
use regex::Regex;
use std::{
    collections::HashMap,
    sync::{Arc, RwLock},
...
lazy_static! {
    ...
    static ref SINGLE_INSTANCE: RwLock<LinkPortalBackendForwarderManager> = RwLock::new(LinkPortalBackendForwarderManager::new());
}

pub struct LinkPortalBackendForwarderManager {
    pub implementations: HashMap<String, Arc<dyn IForwarder + Send + Sync>>,
}

impl LinkPortalBackendForwarderManager {
    fn new() -> Self {
        LinkPortalBackendForwarderManager {
            implementations: HashMap::new(),
        }
    }

    pub fn get() -> &'static RwLock<LinkPortalBackendForwarderManager> {
        &SINGLE_INSTANCE
    }
    ...
    pub async fn init() {
        ...
        tracing::info!("Register LinkPortalBackend Http IForwarder ...");
        match Self::get()
        ...
        {
            Ok(registered) => {
                tracing::info!("Initializing LinkPortalBackend Http IForwarder ...");
    ...
    pub fn get_implementation(name: String) -> Result<Arc<dyn IForwarder + Send + Sync>, Error> {
        ...
        let this = LinkPortalBackendForwarderManager::get().read().unwrap();
    ...
    pub async fn linkportalbackend_middleware(
        ...
        if ipfilter.is_blocked(incoming.to_owned()).await.unwrap_or(false) {
            ...
            return Response::builder()
                .status(code)
                .body("Access denied by LinkPortalBackend IP Filter".into())
                .unwrap();
        }
        ...
        if let Some(intervention) = transaction.intervention() {
            if intervention.status() == 401 || intervention.status() == 403 {
                ...
                let logmsg = intervention
                    ...
                    .unwrap_or_else(|| "Access denied by LinkPortalBackend".to_string());
                tracing::info!(
                    "[LinkPortalBackend] [AccessDeined] - {}, reason: {}",
                    incoming.path,
                    logmsg
                );
                ...
                return Response::builder()
                    ...
                    .body("Access denied by LinkPortalBackend Threaten".into())
                    .unwrap();
            }
        }

        // Forwarding request to the upstream servers.
        let forwarder = LinkPortalBackendForwarderManager::get_implementation(HttpForwardHandler::NAME.to_owned())
            .expect(&format!(
        ...
        match forwarder.http_forward(incoming.to_owned()).await {
            std::result::Result::Ok(response) => {
                tracing::info!("[LinkPortalBackend] [Forwarded] - {}", &incoming.path);
                response
            }
            Err(err) => {
                tracing::warn!("[LinkPortalBackend] [ForwardErr] - {} - {}", &incoming.path, err);
